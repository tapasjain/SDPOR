...
// tapas, vansh -> changed the first run according to our implemented SDPOR.
...
// tapas, vansh
// This function calculates the I(state, w) and save it in ans
// It contains all those processes(p) in w for which there is no event e in dom(state, w) such that e happens before next(state, p) 
void Scheduler::return_min(State * state, vector<InspectEvent> &w, set<int> &ans)
{
	for (vector<InspectEvent>::iterator it = w.begin(); it!= w.end() ;++it)
	{
		InspectEvent nextEp = state->prog_state->enabled.get_transition(it->thread_id);
		if (nextEp.valid())
		{
...
//			cout << "Checking against \n";
			int tobeadded = 1;
			for (vector<InspectEvent>::iterator it2 = w.begin(); it2!= w.end() ;++it2)
			{
...
//				verbose(-1e9, e.toString());
				...
				if(occured_before && (e.dependent(nextEp) || e.thread_id==nextEp.thread_id)) {
					tobeadded = 0;
					break;
				}
			}
...
void Scheduler::utility_loop(State *E, State * old_state, InspectEvent &event, InspectEvent *event_at_E_) {

	vector<InspectEvent> thread_ids_notdep;
	State *new_state = old_state->next;
	while (new_state != E)
	{
		if (not (old_state->sel_event.dependent(new_state->sel_event) || old_state->sel_event.thread_id == new_state->sel_event.thread_id)){
	...
	for (set<int>::iterator it = minimal_processes.begin(); it!= minimal_processes.end(); ++it)
	{
		InspectEvent nextEp = old_state->prog_state->enabled.get_transition(*it);
		assert(nextEp.valid());
...
// then e is not blocked before the occurrence of event.  (This is done be checking no other event occurs which lies in b/w these 2 in terms of happens-before transition & race reversal)
// If the above condition is true, it calls the utility-loop function. 
void Scheduler::loopmain(State*E, InspectEvent &event) {
	State *old_state = E->prev;
	while(old_state!=NULL) {
		// checking happens before
		if(event.thread_id != old_state->sel_event.thread_id && old_state->sel_event.dependent(event)) {

			State *new_state = old_state->next;
			int incondition = 1;
			while(new_state != E) {
				if((old_state->sel_event.dependent(new_state->sel_event) || old_state->sel_event.thread_id==new_state->sel_event.thread_id)
						&& (new_state->sel_event.dependent(event) || new_state->sel_event.thread_id==event.thread_id)) {
					incondition = 0;
					break;
				}
				new_state = new_state->next;
			...
			if(incondition) {
				InspectEvent alt_event = old_state->prog_state->enabled.get_transition(event.thread_id);
				if(alt_event.invalid()) {
					alt_event = old_state->prog_state->disabled.get_transition(event.thread_id);
					if (alt_event.invalid())
					{
//						cout << "Invalid encountered \n";
				...
				{
					utility_loop(E,old_state, event,& old_state->sel_event);
				}
			}
		}
		old_state = old_state->prev;
