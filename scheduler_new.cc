...
//						cout << "Invalid encountered \n";
				...
				{
					backtrack_SourceDPOR_helper(E,old_state, event,& old_state->sel_event);
				}
			}
		}
		old_state = old_state->prev;
...
{
    State * state = nullptr, *current_state = nullptr, * new_state = nullptr;
...
void Scheduler::SourceDPOR() {
    ...
    // check the state_stack for the last state with backtrack elements in it
	while (state != NULL && state->backtrack.empty()) {
		state_stack.pop();
		//cout << "Printing states to delete ";
		//verbose(-1e9, state->sel_event.toString());
...
//	verbose(-1e9, event.toString());
    ...
    /* actual backtrack function starts here */
	event = state->backtrack.get_transition();
