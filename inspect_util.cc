...
void Socket::init_af_inet()
{
  ...
  setsockopt(sock_fd, SOL_SOCKET, SO_SNDBUF, &buflen, sizeof(buflen));
  setsockopt(sock_fd, SOL_SOCKET, SO_RCVBUF, &buflen, sizeof(buflen));
  assert(sock_fd > 0);
}
...
int Socket::send_int(const int val)
{
  ...
  int retval = ::send(sock_fd, &sval, sizeof(sval), 0);
...
{
  unsigned int buf;
  int retval = ::recv(sock_fd, &buf, sizeof(buf),0);  
...
{
  int retval = ::send(sock_fd, &val, sizeof(val), 0);
...
{
  int retval = ::recv(sock_fd, val, sizeof(double), 0);
...
int Socket::send_string(string str)
{
  ...
  if (len > 0){
    retval = ::send(sock_fd, str.c_str(), len, 0);
...
int Socket::send_binary(const void* data, const int len)
{
  ...
  retval = ::send(sock_fd, data, len, 0);
...
int Socket::recv_binary(void * data)
{
  ...
  retval = ::recv(sock_fd, data, msglen, 0);
...
string  get_hostname_by_ip(string ip_addr)
{
  ...
  hent = gethostbyaddr(&naddr, 4, AF_INET);
...
vector<string> break_string_by_whitespace(string & str)
{
  ...
  unsigned int spos, epos, sbegin, send;
  string sub;
